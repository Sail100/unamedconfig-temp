...
runFunction(function()
	local anticheatspazzer = {}
	anticheatspazzer = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		Name = 'AnticheatSpazzer',
		HoverText = 'spazzes the ac\n(dev)',
		Function = function(calling)
			if calling then
				task.spawn(function()
				    if lplr.name == 'renderwhitelistuser' then
					    InfoNotification('AnticheatSpazzer', 'Saved you from chaos lol.', 5)
					else
						AnticheatBypass.SetValue(true)
						...
						repeat
							...
							makefolder("24")
							makefolder("25")
						until not anticheatspazzer.Enabled
					end
				end)
			end
		end
	})
	anticheatspazzercrash = anticheatspazzer.CreateToggle({
		Name = 'Crash',
		HoverText = 'doesnt crash you :)',
		Default = false,
		Function = function(calling) 
			if calling then 
				while true do
